...
class Z3Wrapper(object):
	...
	def _findModel(self):
		...
		if self.use_lia:
			...
			if self.z3_expr.getStringVars():
				model = self._getModel() if res == sat else None
				self.solver.pop()
				return model
			self.solver.pop()
			if res == unsat:
	...
	def _getModel(self):
		...
		for name in self.z3_expr.z3_vars.keys():
			try:
				ce = model.eval(self.z3_expr.z3_vars[name], model_completion=True)
				if is_string(ce):
					res[name] = ce.as_string()
				elif is_int_value(ce):
