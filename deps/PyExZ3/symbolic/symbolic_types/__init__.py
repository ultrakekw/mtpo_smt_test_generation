...
def getSymbolic(v):
	# bool is represented by the integer symbolic backend because Python bool
	# values are int-compatible and PyExZ3 records branch truthiness separately.
	exported = [(bool,SymbolicInteger),(int,SymbolicInteger),(dict,SymbolicDict),(str,SymbolicStr)]
	for (t,s) in exported:
		if isinstance(v,t):
			return s
	return None
